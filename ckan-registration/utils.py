...
def build_resource_plan(files: list[Path], root: Path, source_url: str) -> list[dict[str, Any]]:
    ...
    for path in files:
        ...
        plan.append(
            {
                "resource_name": name,
                "resource_title": resource_title_from_path(path),
                "resource_description": f"Model file from {rel}.",
...
def get_mint_standard_variables_for_model(
    ...
    *,
    ...
    ids: list[str] = []
    names: list[str] = []
    source = "custom/models/standard_variable"
    ...
    names = [x for x in dict.fromkeys(names)]
    ...
    if names:
        ...
        ids = [x for x in dict.fromkeys(ids + resolved_ids)]

    if not ids and fallback_model_standard_variable_names:
        source = "fallback_standard_variable_names"
        fallback_names = [clean_text(x) for x in fallback_model_standard_variable_names if clean_text(x)]
        names = [x for x in dict.fromkeys(fallback_names)]
        resolved_ids, unresolved_from_names = resolve_mint_standard_variable_ids(
            names,
            mint_api_base=mint_api_base,
            mint_api_token=mint_api_token,
        ...
        ids = [x for x in dict.fromkeys(resolved_ids)]
